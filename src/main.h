...
static const unsigned int MAX_STANDARD_TX_SIZE = MAX_BLOCK_SIZE/4;
...
public:
    ...
    {
        if (cUnit == 'S')
            return (vin.size() == 1 && vin[0].prevout.hash == 0 && vin[0].prevout.n == (unsigned int) -2 && vout.size() >= 1);
        else
    ...
    */
    bool AreInputsSameUnit(CCoinsViewCache& mapInputs) const;
    ...
    */
    ...
    // nubit: return the min fee in a block when we only know its parent
    int64 GetUnitMinFeeFromPrevBlock(const CBlockIndex *pindexPrev) const;
    int64 GetMinFeeFromPrevBlock(const CBlockIndex *pindexPrev) const;
    int64 GetSafeMinFeeFromPrevBlock(const CBlockIndex *pindexPrev) const;
    ...
    std::string ToString() const
    {
        ...
        str += strprintf("(hash=%s, unit=%c, nTime=%u, ver=%d, vin.size=%"PRIszu", vout.size=%"PRIszu", nLockTime=%u)\n",
...
public:
    ...
    unsigned char cUnit;  // nubit: if the outpoint was the last unspent: its unit
    

    CTxInUndo() : txout(), fCoinBase(false), nHeight(0), nVersion(0), fCoinStake(false), nTime(0), cUnit('?') {}
    CTxInUndo(const CTxOut &txoutIn, bool fCoinBaseIn = false, unsigned int nHeightIn = 0, int nVersionIn = 0, bool fCoinStakeIn = false, unsigned int nTimeIn = 0, unsigned char cUnit = '?') : txout(txoutIn), fCoinBase(fCoinBaseIn), nHeight(nHeightIn), nVersion(nVersionIn), fCoinStake(fCoinStakeIn), nTime(nTimeIn), cUnit(cUnit) { }
    ...
    unsigned int GetSerializeSize(int nType, int nVersion) const {
        return ::GetSerializeSize(VARINT(nHeight*4+(fCoinBase ? 1 : 0)+(fCoinStake ? 2 : 0)), nType, nVersion) +
               ...
               ::GetSerializeSize(VARINT(cUnit), nType, nVersion) +
    ...
    void Serialize(Stream &s, int nType, int nVersion) const {
        ...
        ::Serialize(s, VARINT(cUnit), nType, nVersion);
        if (nHeight > 0)
    ...
    void Unserialize(Stream &s, int nType, int nVersion) {
        ...
        ::Unserialize(s, VARINT(cUnit), nType, nVersion);
        if (nHeight > 0)
...
public:
    ...
    // nubit: transaction unit
    unsigned char cUnit;

    // construct a CCoins from a CTransaction, at a given height
    CCoins(const CTransaction &tx, int nHeightIn) : fCoinBase(tx.IsCoinBase()), vout(tx.vout), nHeight(nHeightIn), nVersion(tx.nVersion), fCoinStake(tx.IsCoinStake()), nTime(tx.nTime), cUnit(tx.cUnit) { }

    // empty constructor
    CCoins() : fCoinBase(false), vout(0), nHeight(0), nVersion(0), fCoinStake(false), nTime(0), cUnit('?') { }
    ...
    unsigned int GetSerializeSize(int nType, int nVersion) const {
        ...
        // nubit unit
        nSize += ::GetSerializeSize(VARINT(cUnit), nType, nVersion);
        return nSize;
    }
