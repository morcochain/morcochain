        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            loop
            {
                ...
                BOOST_FOREACH (const PAIRTYPE(CScript, int64)& s, vecSend)
                {
                    ...
                    // nubit: split shares if appropriate
...
bool CWallet::CreateBurnTransaction(int64 nBurnValue, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl)
{
    if (nBurnValue < 0)
    {
        strFailReason = _("Invalid burn amount");
        return false;
    }
...
static map<const CWalletTx*, CDiskTxPos> mapDiskTxPosCache;
static map<const CWalletTx*, CBlockHeader> mapTxBlockHeaderCache;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew, CBlockIndex* pindexprev)
{
    ...
    while (it != mapTxLastUse.end())
    {
        ...
        {
            mapTxHash.erase(wtx);
            mapDiskTxPosCache.erase(wtx);
            mapTxBlockHeaderCache.erase(wtx);
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        map<const CWalletTx*, CDiskTxPos>::const_iterator itTxIndex = mapDiskTxPosCache.find(pcoin.first);
        if (itTxIndex == mapDiskTxPosCache.end())
        {
        ...
        // Read block header
        // nubit: cache block header
        map<const CWalletTx*, CBlockHeader>::const_iterator itTxBlock = mapTxBlockHeaderCache.find(pcoin.first);
        if (itTxBlock == mapTxBlockHeaderCache.end())
        {
            pair<const CWalletTx*, CBlockHeader> value(pcoin.first, CBlockHeader());
            ...
            itTxBlock = mapTxBlockHeaderCache.insert(value).first;
        }
        const CBlockHeader& header = itTxBlock->second;
...

    {
        ...
        CValidationState state;
        if (!txNew.GetCoinAge(state, view, vote.nCoinAgeDestroyed))
...
string CWallet::SendMoney(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    if (!CreateTransaction(scriptPubKey, nValue, wtxNew, reservekey, nFeeRequired, strError))
    {
        if (nValue + nFeeRequired > GetBalance())
            strError = strprintf(_("Error: This transaction requires a transaction fee of at least %s."), FormatMoney(nFeeRequired).c_str());
        printf("SendMoney() : %s\n", strError.c_str());
        return strError;
    }

    if (fAskFee && !uiInterface.ThreadSafeAskFee(nFeeRequired, cUnit))
        return "ABORTED";
...
string CWallet::BurnMoney(int64 nValue, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    string strError;
    if (!CreateBurnTransaction(nValue, wtxNew, reservekey, nFeeRequired, strError))
    {
        if (nValue + nFeeRequired > GetBalance())
            strError = strprintf(_("Error: This transaction requires a transaction fee of at least %s."), FormatMoney(nFeeRequired).c_str());
        printf("SendMoney() : %s\n", strError.c_str());
...
void CWallet::ExportPeercoinKeys(int &nExportedCount, int &nErrorCount)
{
    ...
    BOOST_FOREACH(const PAIRTYPE(CTxDestination, string)& item, mapAddressBook)
    {
        ...
        if (address.IsScript(cUnit))
        {
            ...
            try
            {
                std::string result = write_string(json_spirit::Value(CallPeercoinRPC("addmultisigaddress", params)), false);
        ...
        else
        {
            ...
            try
            {
                string result = write_string(json_spirit::Value(CallPeercoinRPC("importprivkey", params)), false);
