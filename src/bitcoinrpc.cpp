...
map<unsigned char, asio::io_service*> rpc_io_service;
map<unsigned char, ssl::context*> rpc_ssl_context;
map<unsigned char, boost::thread_group*> rpc_worker_group;

// nubit: one wallet per RPC thread.
// pwalletMain is the wallet of the current thread
...
static inline unsigned short GetPeercoinDefaultRPCPort()
{
    return GetBoolArg("-testnet", false) ? PEERCOIN_TESTNET_RPC_PORT : PEERCOIN_RPC_PORT;
}
...
{ //  name                      actor (function)         okSafeMode threadSafe
  //  ------------------------  -----------------------  ---------- ----------
    ...
    { "getfee",                 &getfee,                 false,     false },
    { "gettxoutsetinfo",        &gettxoutsetinfo,        true,      false },
    { "gettxout",               &gettxout,               true,      false },
    { "lockunspent",            &lockunspent,            false,     false },
    { "listlockunspent",        &listlockunspent,        false,     false },
    // nubit:
    { "getparkrates",           &getparkrates,           true,      false },
    { "park",                   &park,                   false,     false },
    { "unpark",                 &unpark,                 false,     false },
    { "listparked",             &listparked,             false,     false },
    { "burn",                   &burn,                   false,     false },
    { "getpremium",             &getpremium,             true,      false },
    { "getpeercoinaddresses",   &getpeercoinaddresses,   true,      false },
    { "exportpeercoinkeys",     &exportpeercoinkeys,     false,     false },
    { "distribute",             &distribute,             true,      false },
    { "liquidityinfo",          &liquidityinfo,          false,     false },
    { "getliquidityinfo",       &getliquidityinfo,       false,     false },
    { "getliquiditydetails",    &getliquiditydetails,    false,     false },
    { "getvote",                &getvote,                true,      false },
    { "setvote",                &setvote,                true,      false },
    { "getmotions",             &getmotions,             true,      false },
    { "getcustodianvotes",      &getcustodianvotes,      true,      false },
    { "getelectedcustodians",   &getelectedcustodians,   true,      false },
    { "getparkvotes",           &getparkvotes,           true,      false },
    { "setdatafeed",            &setdatafeed,            true,      false },
    { "getdatafeed",            &getdatafeed,            true,      false },
#ifdef TESTING
    { "shutdown",               &stop,                   true,      true },
    ...
    // nubit:
    { "manualunpark",           &manualunpark,           true,      false },
#endif
};
...
int GetRPCPort(unsigned char cUnit)
{
    ...
    port = GetArg("-rpcport", GetDefaultRPCPort());
...
void RunWalletRPCThread(asio::io_service *service, CWallet* pwallet)
{
    threadWallet.reset(new CWallet*(pwallet));
    service->run();
}

void StartWalletRPCThreads(CWallet* pwallet)
{
    unsigned char cUnit = pwallet->GetUnit();
    // getwork/getblocktemplate mining rewards paid here:
    if (cUnit == 'S')
        pMiningKey = new CReserveKey(pwallet);
    ...
    assert(rpc_io_service[cUnit] == NULL);
    rpc_io_service[cUnit] = new asio::io_service();
    rpc_ssl_context[cUnit] = new ssl::context(*rpc_io_service[cUnit], ssl::context::sslv23);

    const bool fUseSSL = GetBoolArg("-rpcssl");

    if (fUseSSL)
    {
        rpc_ssl_context[cUnit]->set_options(ssl::context::no_sslv2 | ssl::context::no_sslv3);

        filesystem::path pathCertFile(GetArg("-rpcsslcertificatechainfile", "server.cert"));
        if (!pathCertFile.is_complete()) pathCertFile = filesystem::path(GetDataDir()) / pathCertFile;
        if (filesystem::exists(pathCertFile)) rpc_ssl_context[cUnit]->use_certificate_chain_file(pathCertFile.string());
        else printf("ThreadRPCServer ERROR: missing server certificate file %s\n", pathCertFile.string().c_str());

        filesystem::path pathPKFile(GetArg("-rpcsslprivatekeyfile", "server.pem"));
        if (!pathPKFile.is_complete()) pathPKFile = filesystem::path(GetDataDir()) / pathPKFile;
        if (filesystem::exists(pathPKFile)) rpc_ssl_context[cUnit]->use_private_key_file(pathPKFile.string(), ssl::context::pem);
        else printf("ThreadRPCServer ERROR: missing server private key file %s\n", pathPKFile.string().c_str());

        string strCiphers = GetArg("-rpcsslciphers", "TLSv1+HIGH:!SSLv2:!aNULL:!eNULL:!AH:!3DES:@STRENGTH");
        SSL_CTX_set_cipher_list(rpc_ssl_context[cUnit]->impl(), strCiphers.c_str());
    }

    // Try a dual IPv6/IPv4 socket, falling back to separate IPv4 and IPv6 sockets
    const bool loopback = !mapArgs.count("-rpcallowip");
    asio::ip::address bindAddress = loopback ? asio::ip::address_v6::loopback() : asio::ip::address_v6::any();
    ip::tcp::endpoint endpoint(bindAddress, GetRPCPort(cUnit));
    boost::system::error_code v6_only_error;
    boost::shared_ptr<ip::tcp::acceptor> acceptor(new ip::tcp::acceptor(*rpc_io_service[cUnit]));
    ...
    try
    {
        ...
        RPCListen(acceptor, *rpc_ssl_context[cUnit], fUseSSL);
    ...
    try {
        ...
        if (!fListening || loopback || v6_only_error)
        {
            ...
            acceptor.reset(new ip::tcp::acceptor(*rpc_io_service[cUnit]));
            acceptor->open(endpoint.protocol());
            acceptor->set_option(boost::asio::ip::tcp::acceptor::reuse_address(true));
            acceptor->bind(endpoint);
            acceptor->listen(socket_base::max_connections);
    ...
    rpc_worker_group[cUnit] = new boost::thread_group();
    for (int i = 0; i < GetArg("-rpcthreads", 4); i++)
        rpc_worker_group[cUnit]->create_thread(boost::bind(&RunWalletRPCThread, rpc_io_service[cUnit], pwallet));
}
...
void StartRPCThreads()
{
    ...
    {
        StartWalletRPCThreads(GetWallet(cUnit));
    }
}

void StopWalletRPCThreads(unsigned char cUnit)
{
    if (cUnit == 'S')
    {
    ...
    if (rpc_io_service[cUnit] == NULL) return;

    rpc_io_service[cUnit]->stop();
    if (rpc_worker_group[cUnit])
    {
        rpc_worker_group[cUnit]->join_all();
        delete rpc_worker_group[cUnit]; rpc_worker_group[cUnit] = NULL;
    }
    delete rpc_ssl_context[cUnit]; rpc_ssl_context[cUnit] = NULL;
    delete rpc_io_service[cUnit]; rpc_io_service[cUnit] = NULL;
}

void StopRPCThreads()
{
    BOOST_FOREACH(unsigned char cUnit, sAvailableUnits)
        StopWalletRPCThreads(cUnit);
}
...
Object CallPeercoinRPC(const std::string &strMethod, const Array &params)
{
    ...
    SSLIOStreamDevice<asio::ip::tcp> d(sslStream, fUseSSL);
    iostreams::stream< SSLIOStreamDevice<asio::ip::tcp> > stream(d);
    if (!d.connect(GetPeercoinArg("-rpcconnect", "127.0.0.1"), GetPeercoinArg("-rpcport", itostr(GetPeercoinDefaultRPCPort()))))
    ...
    Value valReply;
    if (!read_string(strReply, valReply))
        throw runtime_error("couldn't parse reply from peercoin server");
    const Object& reply = valReply.get_obj();
    if (reply.empty())
        throw runtime_error("expected reply to have result, error and id properties");
...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    if (strMethod == "getfee"                 && n > 0) ConvertTo<boost::int64_t>(params[0]);
    if (strMethod == "getfee"                 && n > 1) ConvertTo<double>(params[1]);
