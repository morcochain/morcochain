...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("trust",         pindexBest->nChainTrust.ToString()));
#endif
    return obj;
}
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(CBitcoinAddress(address, pwalletMain->GetUnit()).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CBitcoinAddress(address, pwalletMain->GetUnit()).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CBitcoinAddress(address, pwalletMain->GetUnit()).Get())->second);
            }
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    ...
    {
        const CWalletTx& wtx = (*it).second;
        if (wtx.IsCoinBase() || !wtx.IsFinal() || wtx.IsCustodianGrant())
            continue;
...
void ThreadTopUpKeyPool(void* parg)
{
    ...
    RenameThread("nu-key-top");

    // nubit: get wallet from arg
    CWallet* pwallet = (CWallet*)parg;
    pwallet->TopUpKeyPool();
}

struct ThreadCleanWalletPassphraseArg
{
    int64 nSleepTime;
    CWallet* pwallet;
};

void ThreadCleanWalletPassphrase(void* pargVoid)
{
    // Make this thread recognisable as the wallet relocking thread
    RenameThread("nu-lock-wa");

    // nubit: get wallet and time from thread arg
    ThreadCleanWalletPassphraseArg* pargStruct = (ThreadCleanWalletPassphraseArg*)pargVoid;
    threadWallet.reset(new CWallet*(pargStruct->pwallet));
    void* parg = &pargStruct->nSleepTime;
    ...
    delete (ThreadCleanWalletPassphraseArg*)pargVoid;
}
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // nubit: pass wallet to threads
    NewThread(ThreadTopUpKeyPool, &(*pwalletMain));
    ThreadCleanWalletPassphraseArg* pargCleanWallet = new ThreadCleanWalletPassphraseArg();
    pargCleanWallet->nSleepTime = params[1].get_int64();
    pargCleanWallet->pwallet = &(*pwalletMain);
    NewThread(ThreadCleanWalletPassphrase, pargCleanWallet);
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; Nu server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
