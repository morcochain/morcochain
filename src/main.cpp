...
{
    // nubit: no dust, a minimum fee is always enforced
    return false;
}
...
bool CTransaction::AreInputsSameUnit(CCoinsViewCache& mapInputs) const
{
    for (unsigned int i = 0; i < vin.size(); i++)
    {
        const CTxIn& input = vin[i];
        const CCoins &coins = mapInputs.GetCoins(input.prevout.hash);
        if (!coins.IsAvailable(input.prevout.n))
            throw std::runtime_error("CTransaction::AreInputsSameUnit() : prevout not found");

        if (coins.cUnit != cUnit)
            return false;
    }
...
bool CTransaction::CheckTransaction(CValidationState &state) const
{
    ...
    if (!IsValidUnit(cUnit))
        return state.DoS(10, error("CTransaction::CheckTransaction() : invalid unit"));
    if (vin.empty())
    ...
    if (cUnit != 'S' && (IsCoinBase() || IsCoinStake()))
        return state.DoS(10, error("CTransaction::CheckTransaction() : invalid unit in CoinBase or CoinStake"));
    ...
    if (IsCoinStake() && ::GetSerializeSize(*this, SER_NETWORK, PROTOCOL_VERSION) > MAX_COINSTAKE_SIZE)
        return state.DoS(100, error("CTransaction::CheckTransaction() : coinstake size"));
    ...
    else if (IsCustodianGrant())
    {
        ...
        if (vin[0].scriptSig.size() != 0)
            return state.DoS(100, error("CTransaction::CheckTransaction() : currency coinbase script size"));
    }
    else
    {
        BOOST_FOREACH(const CTxIn& txin, vin)
    ...
    for (unsigned int i = 0; i < vout.size(); i++)
    {
        ...
        if (IsPark(txout.scriptPubKey))
        {
            ...
            if (cUnit == 'S')
                return state.DoS(100, error("CTransaction::CheckTransaction() : parking of shares"));

            if (!IsValidPark(txout.scriptPubKey))
                return state.DoS(100, error("CTransaction::CheckTransaction() : invalid parking transaction"));
        }
    }
...
int64 CTransaction::GetUnitMinFeeFromPrevBlock(const CBlockIndex *pindexPrev) const
{
    const CBlockIndex dummyIndex;
    const_cast<CBlockIndex*>(&dummyIndex)->pprev = const_cast<CBlockIndex*>(pindexPrev);
    return GetUnitMinFee(&dummyIndex);
}
int64 CTransaction::GetMinFeeFromPrevBlock(const CBlockIndex *pindexPrev) const
{
    const CBlockIndex dummyIndex;
    const_cast<CBlockIndex*>(&dummyIndex)->pprev = const_cast<CBlockIndex*>(pindexPrev);
    return GetMinFee(&dummyIndex);
}
int64 CTransaction::GetSafeMinFeeFromPrevBlock(const CBlockIndex *pindexPrev) const
{
    const CBlockIndex dummyIndex;
    const_cast<CBlockIndex*>(&dummyIndex)->pprev = const_cast<CBlockIndex*>(pindexPrev);
    return GetSafeMinFee(&dummyIndex);
}
                        ...
                        bool* pfMissingInputs)
{
    ...
    if (tx.IsCustodianGrant())
        return state.DoS(100, error("CTxMemPool::accept() : currency coinbase as individual tx"));
...
bool CTransaction::CheckInputs(CValidationState &state, CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, std::vector<CScriptCheck> *pvChecks) const
{
    if (!IsCoinBase() && !IsCustodianGrant())
    {
        ...
        const CBlockIndex* pindexPrev = inputs.GetBestBlock();
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            if (coins.IsCoinBase() || coins.IsCoinStake()) {
                if (nSpendHeight - coins.nHeight < GetMaturity(coins.IsCoinStake()))
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            if (IsUnpark())
            {
                ...
                if (!coins.IsAvailable(prevout.n))
                    return error("ConnectInputs() : prevout is spent");

                const CTxOut& txo = coins.vout[prevout.n];
                ...
                CBitcoinAddress unparkAddress(unparkDestination, coins.cUnit);

                if (nSpendHeight - coins.nHeight < nDuration)
                    return error("ConnectInputs() : parking duration has not passed");

                const CBlockIndex *pindexPark = pindexPrev;
                for (; pindexPark && pindexPark->nHeight != coins.nHeight; pindexPark = pindexPark->pprev) {};
                if (!pindexPark)
                    return error("ConnectInputs() : parked transaction not in chain");

                int64 nValue = txo.nValue;
        ...
        if (IsCoinStake())
        {
            ...
            if (nStakeReward > GetProofOfStakeReward() - GetMinFeeFromPrevBlock(pindexPrev) + GetUnitMinFeeFromPrevBlock(pindexPrev))
        ...
        else if (!fValidUnpark)
        {
            ...
            if (nTxFee < GetMinFeeFromPrevBlock(pindexPrev))
                return state.DoS(100, error("CheckInputs() : %s not paying required fee=%s, paid=%s", GetHash().ToString().c_str(), FormatMoney(GetMinFeeFromPrevBlock(pindexPrev)).c_str(), FormatMoney(nTxFee).c_str()));
            nFees += nTxFee;
            if (!MoneyRange(nFees))
        ...
        // nubit: unpark transactions are not signed
        if (fScriptChecks && !fValidUnpark) {
            for (unsigned int i = 0; i < vin.size(); i++) {
                const COutPoint &prevout = vin[i].prevout;
                const CCoins &coins = inputs.GetCoins(prevout.hash);
...
bool CBlock::DisconnectBlock(CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &view, bool *pfClean)
{
    ...
    for (int i = vtx.size() - 1; i >= 0; i--) {
        ...
        // restore inputs
        if (i > 0 && !tx.IsCustodianGrant()) { // not coinbases // nubit: not custodian grants
...
bool CBlock::CheckCustodianGrants(const CBlockIndex* pindexPrev) const
{
    ...
    if (vActualCurrencyCoinBase.size() != vExpectedCurrencyCoinBase.size())
        return error("CheckCustodianGrants() : unexpected number of expansion transaction");

    for (unsigned int i = 0; i < vActualCurrencyCoinBase.size(); i++)
    {
        ...
        if (actualTx != expectedTx)
        {
            ...
            return error("CheckCustodianGrants() : invalid expansion transaction found");
        }
    }
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
    ...
    BOOST_FOREACH(CBlockIndex* pindex, vDisconnect) {
        ...
        BOOST_REVERSE_FOREACH(const CTransaction& tx, block.vtx)
            if (!(tx.IsCoinBase() || tx.IsCoinStake() || tx.IsCustodianGrant()) && pindex->nHeight > Checkpoints::GetTotalBlocksEstimate())
    ...
    nTransactionsUpdated++;
    printf("SetBestChain: new best=%s  height=%d  log2_trust=%.8g  moneysupply(S)=%s  moneysupply(B)=%s  tx=%lu  date=%s progress=%f\n",
      hashBestChain.ToString().c_str(), nBestHeight, log(nBestChainTrust.getdouble())/log(2.0),
      FormatMoney(pindexBest->GetMoneySupply('S')).c_str(), FormatMoney(pindexBest->GetMoneySupply('B')).c_str(),
...
bool CBlock::GetCoinStakeAge(uint64& nCoinAge) const
{
    ...
    CCoinsViewCache view(*pcoinsTip, true);
    CValidationState state;
    return vtx[1].GetCoinAge(state, view, nCoinAge);
}
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        // nubit: rule delayed until next protocol update (which is not known yet)
        if (false && nVersion < 2)
        {
...
bool LoadBlockIndex()
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nCoinstakeMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Nu", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nCoinstakeMaturity, nModifierInterval);
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (!fAlreadyHave) {
                ...
                {
                    if (mapAlreadyAskedFor.count(inv) && IsInitialBlockDownload())
...
CBlockTemplate* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            int64 nMinFee = tx.GetMinFeeFromPrevBlock(pindexPrev);
...
#endif
{
    ...
    RenameThread(fProofOfStake? "nu-stake-minter" : "nu-miner");
    ...
    try { loop {
        ...
        if (fProofOfStake)
        {
            ...
            if (pblock->IsProofOfStake())
            {
                ...
                bool fSuccess = CheckWork(pblock, *pwallet, reservekey);
                SetThreadPriority(THREAD_PRIORITY_LOWEST);
