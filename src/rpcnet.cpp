...
Value makekeypair(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            "makekeypair [prefix] [<unit>]\n"
            "Make a public/private key pair.\n"
            "[prefix] is optional preferred prefix for the public key.\n"
            "You can provide an <unit> to get the private key in its base58 format.\n");
    ...
    if (cUnit != '?' && cUnit != 0)
        result.push_back(Pair("PrivateKey", CBitcoinSecret(vchSecret, fCompressed, cUnit).ToString()));
