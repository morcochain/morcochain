...
BOOST_AUTO_TEST_CASE(base58_keys_valid_parse)
{
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        if(isPrivkey)
        {
            ...
            BOOST_CHECK_MESSAGE(secret.SetString(exp_base58string, 'S'), "!SetString:"+ strTest);
            BOOST_CHECK_MESSAGE(secret.IsValid('S'), "!IsValid:" + strTest);
        ...
        else
        {
            ...
            secret.SetString(exp_base58string, 'S');
            BOOST_CHECK_MESSAGE(!secret.IsValid('S'), "IsValid pubkey as privkey:" + strTest);
        }
    }
...
BOOST_AUTO_TEST_CASE(base58_keys_valid_gen)
{
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        if(isPrivkey)
        {
            bool isCompressed = find_value(metadata, "isCompressed").get_bool();
            CBitcoinSecret secret;
            secret.SetSecret(CSecret(exp_payload.begin(), exp_payload.end()), isCompressed, 'S');
        ...
        else
        {
            ...
            BOOST_CHECK_MESSAGE(boost::apply_visitor(CBitcoinAddressVisitor(&addrOut, 'S'), dest), "encode dest: " + strTest);
    ...
    BOOST_CHECK(!boost::apply_visitor(CBitcoinAddressVisitor(&dummyAddr, 'S'), nodest));
...
BOOST_AUTO_TEST_CASE(base58_keys_invalid)
{
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        secret.SetString(exp_base58string, 'S');
        BOOST_CHECK_MESSAGE(!secret.IsValid('S'), "IsValid privkey:" + strTest);
    }
}
