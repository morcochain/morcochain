...
void WalletStack::changeUnit(const QString &unit)
{
    WalletView *walletView = (WalletView*)currentWidget();
    if (walletView) walletView->changeUnit(unit);
}

void WalletStack::exportPeercoinKeys()
{
    WalletView *walletView = (WalletView*)currentWidget();
    if (walletView) walletView->exportPeercoinKeys();
}

unsigned char WalletStack::getUnit() const
{
    WalletView *walletView = (WalletView*)currentWidget();
    if (walletView)
        return walletView->getUnit();
    return '?';
}

bool WalletStack::isUnlockedForMintingOnly() const
{
    WalletView *walletView = (WalletView*)currentWidget();
    if (walletView)
        return walletView->isUnlockedForMintingOnly();
    return false;
}

void WalletStack::gotoVotePage()
{
    QMap<QString, WalletView*>::const_iterator i;
    for (i = mapWalletViews.constBegin(); i != mapWalletViews.constEnd(); ++i)
        i.value()->gotoVotePage();
}

void WalletStack::gotoParkPage()
{
    QMap<QString, WalletView*>::const_iterator i;
    for (i = mapWalletViews.constBegin(); i != mapWalletViews.constEnd(); ++i)
