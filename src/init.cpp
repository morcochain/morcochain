...
void Shutdown()
{
    ...
    StopNode();
    {
        ...
        {
            CWallet* pwalletMain = GetWallet(cUnit);
            if (pwalletMain)
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    {
        InitWarning(_("Warning: -paytxfee is not used in Nu."));
    }
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Nu is probably already running."), strDataDir.c_str()));
    ...
    BOOST_FOREACH(unsigned char cUnit, sAvailableUnits) {
        ...
        nStart = GetTimeMillis();
        bool fFirstRun = true;
        string walletFilename((format("wallet%c.dat") % cUnit).str());
        CWallet* pwalletMain;
        pwalletMain = new CWallet(walletFilename);
        pwalletMain->SetUnit(cUnit);
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Nu") << "\n";
...
#endif
    MintStake(threadGroup, GetWallet('S'));
